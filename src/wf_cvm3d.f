...
c version d fixes near surface velocities near boreholes
         include 'in.h'
         include 'surface.h'
...
9898     continue
         call makereg(rlat(l0),rlon(l0),rdep(l0),alp,bet,iregfl)
         alphatomo=alp
         betatomo=bet
...
c     depths in meters
         ...
         kerr=0
         illag=0
         open(15,file='points_in',status='old',err=1099)
         read(15,*)nn
         do 150 i=1,nn
c getting different files with lat-long or long-lat
         read(15,*)rfi1,rfi2,rdep(i)
         if(rfi1.lt.0.)then
         rlon(i)=rfi1
         rlat(i)=rfi2
         else
...
c        read(15,*)rlat(i),rlon(i),rdep(i)
c        rlon(i)=rlon(i)*(-1.)
c        read(15,*)rlon(i),rlat(i),rdep(i)
...
c---loop to read-------------------
         do 117 i=1,numsur
          ...
          open(16,file=aname,status='old',err=99)
           do 118 k=1,nlasur(i)
           do 118 j=1,nlosur(i)
...
c  nregly = number layers in regional model
         ...
         include 'regional.h'
          kerr=0
         open(19,file='reg_mod',status='old',err=2999)
         do 1119 k=1,nreglat
          do 1119 j=1,nreglon
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
         subroutine getsoil(rlatl0,rlonl0,isoilt,idgens)
...
c -- write out answers --------------------------------
         include 'in.h'
         kerr=0
         open(17,file='points_out',status='new')
         do 155 i=1,nn
          rdep(i)=rdep(i)/3.2808399
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
         subroutine addtopp(rdep,rvelo,idgen,ifs,rveln,r1de)
